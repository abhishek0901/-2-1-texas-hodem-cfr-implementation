...
class Poker:
    ...
    def compare_strength(self,h1,h2):
        ...
        #check for two pairs
        h1_two_pairs = (self.all_same(h1_faces[0:2]) and self.all_same(h1_faces[2:4])) or \
                        (self.all_same(h1_faces[0:2]) and self.all_same(h1_faces[3:5])) or \
                        (self.all_same(h1_faces[1:3]) and self.all_same(h1_faces[3:5]))
        h2_two_pairs = (self.all_same(h2_faces[0:2]) and self.all_same(h2_faces[2:4])) or \
                       (self.all_same(h2_faces[0:2]) and self.all_same(h2_faces[3:5])) or \
                       (self.all_same(h2_faces[1:3]) and self.all_same(h2_faces[3:5]))
        ...
        #check for pair
        h1_pair = self.all_same(h1_faces[0:2]) or self.all_same(h1_faces[1:3]) or \
                  self.all_same(h1_faces[2:4]) or self.all_same(h1_faces[3:5])

        h2_pair = self.all_same(h2_faces[0:2]) or self.all_same(h2_faces[1:3]) or \
                  self.all_same(h2_faces[2:4]) or self.all_same(h2_faces[3:5])
    ...
    def deal_cards(self,x):
        ...
        if x == 1:
            for i in range(4):
                s += self.int_to_card(self.CARDS[i:i+2])
        elif x == 2:
            for i in range(4,7):
                s += self.int_to_card(self.CARDS[i:i+2])
        return s
